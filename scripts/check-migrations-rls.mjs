// Static lint: every table created in supabase/migrations/ must have RLS
// enabled by some migration. Catches the migration-018 failure mode where
// tables shipped without the `enable row level security` boilerplate and
// were left publicly writable via PostgREST.
//
...
console.log(
  `OK: all ${created.size} tables across migrations have RLS enabled.`
);
