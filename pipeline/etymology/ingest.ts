 ...
 *
 * Reads Mahan Kosh definitions that carry a cross_refs.origin_lang marker —
 * since #32 regenerated by pipeline/mahan-kosh/ingest.ts from the structured
 * parse (parse_shorthand.py + abbreviations.json), i.e. canonical printed-key
 * markers only — turns each into an etymology candidate
 * (pipeline/etymology/parse.ts), and for Sanskrit candidates with an
 * extracted Devanagari root, looks the root up in the Monier-Williams
 * dictionary (Cologne C-SALT API) for a cited gloss.
 *
